...
const DProductAdd = ({ cat }) => {
    const [free, setFree] = useState(true);
    const [prodname, setProdName] = useState('')
    const [loading, setLoading] = useState(false)
    const [file, setFile] = useState(null)
    const { diamond_categories } = useContext(UserContext)
    const selectedCategory = diamond_categories?.filter(e => e.catname === cat)[0]
    ...
    const submitProduct = async () => {
        try {
            ...
            const res = await axios.put(`${server}/diamond/add-product/${cat}`, { prodname, prodimg, free }, { withCredentials: true })
            await Swal.fire({
                title: "Successfull",
